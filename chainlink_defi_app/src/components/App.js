...
import ExchangeToken from "../abis/ExchangeToken.json"
...
class App extends Component {
  ...
  async loadBlockchainData() {
    ...
    // Load DappToken
    const exchangeTokenData = ExchangeToken.networks[networkId]
    if (exchangeTokenData) {
      const exchangeToken = new web3.eth.Contract(
        ExchangeToken.abi,
        exchangeTokenData.address
      )
      this.setState({ exchangeTokenAddress: exchangeTokenData.address })
      this.setState({ exchangeToken })
      let exchangeTokenBalance = await exchangeToken.methods
        .balanceOf(this.state.account)
        .call()
      this.setState({ exchangeTokenBalance: exchangeTokenBalance.toString() })
    } else {
      window.alert("ExchangeToken contract not deployed to detected network.")
    }
    ...
    if (this.state.loading) {
      ...
      content = (
        <Main
          ...
          exchangeTokenBalance={this.state.exchangeTokenBalance}
          exchangeTokenAddress={this.state.exchangeTokenAddress}
