...
module.exports = async function(deployer, network, accounts) {
  ...
  await deployer.deploy(TokenFarm, exchangeToken.address);
  const tokenFarm = await TokenFarm.deployed();
  await exchangeToken.transfer(tokenFarm.address, "1000000000000000000000000");
  ...
  if (network.startsWith("kovan") || network.startsWith("live")) {
    ...
    // EXTT Token Address - also dai
    await tokenFarm.addAllowedTokens(exchangeToken.address);
    await tokenFarm.setPriceFeedContract(
